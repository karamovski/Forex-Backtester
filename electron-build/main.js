...
function parseTimestampSmart(dateStr, timeStr) {
...
function parseTick(line, format) {
...
async function runBacktestCore(tickGenerator, signals, strategy, riskConfig, onProgress) {
  if (!signals || signals.length === 0) {
    throw new Error("No signals provided for backtesting");
  }
  const sortedSignals = [...signals].filter((s) => s.timestamp).sort((a, b) => {
      ...
      if (!closeReason && strategy.useMultipleTPs) {
        for (let tpIndex = 0; tpIndex < trade.signal.takeProfits.length; tpIndex++) {
          ...
          if (tpHit) {
            ...
            closeReason = ["tp1", "tp2", "tp3", "tp4"][tpIndex] || "tp1";
            exitPrice = tp;
...
async function* createTickGenerator(filePaths, tickFormat) {
